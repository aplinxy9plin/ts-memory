...
ENLARGEMENT    = 20		# the mosaic image will be this many times wider and taller than the original
	...
	def get_tiles(self):
		...
		for root, subFolders, files in os.walk(self.tiles_directory):
			for tile_name in files:
				...
				if large_tile:
					large_tiles.append({"name": tile_name, "data": large_tile})
					small_tiles.append({"name": tile_name, "data": small_tile})
...
def build_mosaic(result_queue, all_tile_data_large, original_img_large, img_path):
	...
	while True:
		try:
			...
			if img_coords == EOQ_VALUE:
				...
				if len(photos_arr) < 4:
					photos_arr.append(best_fit_tile_index)
	...
	mosaic.save('output/'+img_path)
	print(photos_arr[0], photos_arr[1], photos_arr[2], photos_arr[3])
	print('\nFinished, output is in', 'output/'+img_path)
...
def compose(original_img, tiles, img_path):
	...
	all_tile_data_large = [list(tile['data'].getdata()) for tile in tiles_large]
	all_tile_data_small = [list(tile['data'].getdata()) for tile in tiles_small]
	...
	try:
		...
		Process(target=build_mosaic, args=(result_queue, all_tile_data_large, original_img_large, img_path)).start()
